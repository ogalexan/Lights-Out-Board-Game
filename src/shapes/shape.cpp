...
Shape::Shape(Shader &shader, glm::vec2 pos, glm::vec2 size, struct color color, int key, int state) :
        shader(shader), pos(pos), size(size), color(color), key(key), state(state) {}
...
void Shape::setState(int s) {state = s; }

void Shape::flipState() {
    if(this->state == 0) {
        this->setState(1);
    }
...
int Shape::getState() const {return state;}
